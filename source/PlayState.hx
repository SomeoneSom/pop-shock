...
class PlayState extends MusicBeatState
{
	...
	var screen1:FlxSprite;
	var screen2:FlxSprite;
	...
	override public function create()
	{
		...
		{
			case 'rebooted':
				dialogue = CoolUtil.coolTextFile(Paths.txt('rebooted/dialog'));
			case 'riff-match':
				dialogue = CoolUtil.coolTextFile(Paths.txt('riff-match/dialog'));
			case 'senpai':
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'rebooted' | 'riff-match':
				{
					var repositionShit = -200;
					defaultCamZoom = 1;
					curStage = 'popstage';
					var bg:FlxSprite = new FlxSprite(-300, -150).loadGraphic(Paths.image('Background'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);

					var stageFront:FlxSprite = new FlxSprite(-300, -150).loadGraphic(Paths.image('Background2'));
					...
					var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('box'));
			...
			case 'neon-nits':
				{
					defaultCamZoom = 1;
					curStage = 'popstage2';
					var bg:FlxSprite = new FlxSprite(-300, -150).loadGraphic(Paths.image('Backgroundneonwall'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);

					var screen1:FlxSprite = new FlxSprite(150, -300);
					screen1.frames = Paths.getSparrowAtlas('screen');
					screen1.animation.addByPrefix('screen', "screen", 24);
					screen1.animation.play('screen');
					screen1.scrollFactor.set(0.9, 0.9);
					var screen2:FlxSprite = new FlxSprite(450, -350);
					screen2.frames = Paths.getSparrowAtlas('screenh');
					screen2.animation.addByPrefix('screenh', "screenh", 24);
					screen2.animation.play('screenh');
					screen2.scrollFactor.set(0.9, 0.9);
					var lights1:FlxSprite = new FlxSprite(150, -300);
					lights1.frames = Paths.getSparrowAtlas('lights1');
					lights1.animation.addByPrefix('lightsp', "lightsp", 24);
					lights1.animation.play('lightsp');
					lights1.scrollFactor.set(0.9, 0.9);
					add(screen1);
					add(screen2);
					var stageFront:FlxSprite = new FlxSprite(-300, -150).loadGraphic(Paths.image('Background2neon'));
					stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
					stageFront.updateHitbox();
					stageFront.antialiasing = true;
					stageFront.scrollFactor.set(0.9, 0.9);
		...
		if (curStage == "popstage2")
		{
			var lights1:FlxSprite = new FlxSprite(0, 0);
			lights1.frames = Paths.getSparrowAtlas('lights1');
			lights1.animation.addByPrefix('lightsp', "lightsp", 24);
			lights1.animation.play('lightsp');
			lights1.scrollFactor.set(0.9, 0.9);
			var crowd2:FlxSprite = new FlxSprite(250, 730);
			crowd2.frames = Paths.getSparrowAtlas('crowd2');
			crowd2.animation.addByPrefix('crowd', "crowd", 24);
			crowd2.animation.play('crowd');
			crowd2.scrollFactor.set(0.9, 0.9);
			crowd2.setGraphicSize(Std.int(crowd2.width * 1.8));
	...
	//mod stuff part 2
	var ampQueue:Array<AmpStruct> = [];
	var ringQueue:Array<RingStruct> = [];
	...
	function startCountdown():Void
	{
		...
		if (executeModchart) // dude I hate lua (jkjkjkjk)
		{
			...
			Lua_helper.add_callback(lua, "moveWindow", function(x:Int, y:Int, sx:Int, sy:Int) {
				Lib.application.window.move(x, y);
				Lib.application.window.resize(sx, sy);
			});

			Lua_helper.add_callback(lua, "moveWindowRel", function(x:Int, y:Int) {
				var ox:Int = Lib.application.window.x;
				var oy:Int = Lib.application.window.y;
				Lib.application.window.move(ox + x, oy + y);
			});

			/*trace(Lua_helper.add_callback(lua, "ringcall", function(startStep2:Float, rotations:Float, stepsPerRotation:Float, startPos:Float, rotationVal:Float, radius:Int) {
				trace("b");
				var start3:Float = Conductor.stepCrochet * startStep2;
				trace("c");
				var end3:Float = start3 + Conductor.stepCrochet * stepsPerRotation * rotations;
				ringQueue.push({startTime : start3, 
					endTime : end3, 
					radInPi : rotations * rotationVal, 
					elapsed : start3, 
					currentRadInPi : startPos, 
					radInPiLeft : rotations * rotationVal, 
					radius : radius
				});
			}));*/
			function ring (startStep2:Float, rotations:Float, stepsPerRotation:Float, startPos:Float, rotationVal:Float, radius:Float) {
				trace("b");
				var start3:Float = Conductor.stepCrochet * startStep2;
				trace("c");
				var end3:Float = start3 + Conductor.stepCrochet * stepsPerRotation * rotations;
			...
			Lua_helper.add_callback(lua, "amp", function(startStep:Float, endStep:Float, relX:Int, relY:Int) {
				var start:Float = startStep * Conductor.stepCrochet;
				var end:Float = endStep * Conductor.stepCrochet;
				var end2:Float = (endStep - startStep) * Conductor.stepCrochet;
				ampQueue.push({startTime : start, 
					endTime : end2 / 10 + start, 
					relX : relX, relY : relY, 
					elapsed : start, 
					relXLeft : relX, 
					relYLeft : relY, 
				...
				ampQueue.push({startTime : end2 / 10 + start, 
					endTime : end, 
					relX : relX * -1, 
					relY : relY * -1, 
					elapsed : end2 / 10 + start, 
					relXLeft : relX * -1, 
					relYLeft : relY * -1, 
					flag : false
				});
			...
			if (PlayState.SONG.song == "neon-nits") {
				ring(448, 15.5, 16, 0.5, -2, ampQueue[4].relY * -2 * 3 / 4);
				ring(696, 0.5, 16, 1.5, -2, ampQueue[4].relY * -2 * 3 / 8);
				ring(960, 23.5, 16, 0.5, 2, ampQueue[4].relY * -2 * 3 / 4);
				ring(1336, 0.5, 16, 1.5, 2, ampQueue[4].relY * -2 * 3 / 8);
			}
		}
		#end
		talking = false;
	...
	function relMove(x:Int, y:Int):Void {
		var ox:Int = Lib.application.window.x;
		var oy:Int = Lib.application.window.y;
		Lib.application.window.move(ox + x, oy + y);
	}
	...
	var xtogostore:Float = 0;
	var ytogostore:Float = 0;
	var xtogostore2:Float = 0;
	var ytogostore2:Float = 0;
	...
	override public function update(elapsed:Float)
	{
		...
		if (executeModchart && lua != null && songStarted)
		{
			...
			if (ampQueue.length != 0) {
				if (Conductor.songPosition >= ampQueue[0].elapsed) {
					var amount:Float;
					var end:Bool = false;
					if (Conductor.songPosition > ampQueue[0].endTime) {
						amount = ampQueue[0].endTime - ampQueue[0].elapsed;
						end = true;
					} else {
						amount = Conductor.songPosition - ampQueue[0].elapsed;
					}
					var outOfOne:Float = amount / (ampQueue[0].endTime - ampQueue[0].startTime);
					if (end) {
						relMove(ampQueue[0].relXLeft, ampQueue[0].relYLeft);
						ampQueue.remove(ampQueue[0]);
						xtogostore = 0;
						ytogostore = 0;
					} else {
						var xtogo:Int = Std.int(ampQueue[0].relX * outOfOne + xtogostore);
						var ytogo:Int = Std.int(ampQueue[0].relY * outOfOne + ytogostore);
						///christ almighty how did i made a 220+ character if statement (almost 250 with spaces)
						if ((ampQueue[0].flag && (ampQueue[0].endTime - ampQueue[0].elapsed) < (ampQueue[0].endTime- ampQueue[0].elapsed) / 3) || (!ampQueue[0].flag && (ampQueue[0].endTime - ampQueue[0].elapsed) > ((ampQueue[0].endTime - ampQueue[0].startTime) * 2 / 3))) {
							relMove(Std.int(ampQueue[0].relX / 4), Std.int(ampQueue[0].relY / 4));
							relMove(Std.int(ampQueue[0].relX / -4 + xtogo), Std.int(ampQueue[0].relY / -4 + ytogo));
						} else {
							relMove(xtogo, ytogo);
						}
						ampQueue[0].relXLeft -= xtogo;
						ampQueue[0].relYLeft -= ytogo;
						var temp:Bool;
						if (xtogo != 0) {
							temp = true;
						} else {
						...
						xtogostore += (ampQueue[0].relX * outOfOne) - xtogo;
						xtogostore -= temp ? Std.int(xtogostore) : 0;
						if (ytogo != 0) {
							temp = true;
						} else {
							temp = false;
						}
						ytogostore += (ampQueue[0].relY * outOfOne) - ytogo;
						ytogostore -= temp ? Std.int(xtogostore) : 0;
						ampQueue[0].elapsed = Conductor.songPosition;
					}
				}
			}
			...
			if (ringQueue.length != 0) {
				if (Conductor.songPosition >= ringQueue[0].elapsed) {
					var amount:Float;
					var end:Bool = false;
					if (Conductor.songPosition > ringQueue[0].endTime) {
						amount = ringQueue[0].endTime - ringQueue[0].elapsed;
						end = true;
					} else {
						amount = Conductor.songPosition - ringQueue[0].elapsed;
					}
					var outOfOne:Float = amount / (ringQueue[0].endTime - ringQueue[0].startTime);
					if (end) {
						var xtogo:Float = ringQueue[0].radius * Math.cos(Math.PI * (ringQueue[0].currentRadInPi + ringQueue[0].radInPiLeft))
						- Math.cos(Math.PI * ringQueue[0].currentRadInPi) * ringQueue[0].radius;
						var ytogo:Float = Math.sin(Math.PI * ringQueue[0].currentRadInPi) * ringQueue[0].radius 
						- ringQueue[0].radius * Math.sin(Math.PI * (ringQueue[0].currentRadInPi + ringQueue[0].radInPiLeft));
						relMove(Std.int(xtogo), Std.int(ytogo));
						ringQueue.remove(ringQueue[0]);
						xtogostore2 = 0;
						ytogostore2 = 0;
					} else {
						var xtogo:Float = ringQueue[0].radius * Math.cos(Math.PI * (ringQueue[0].currentRadInPi + outOfOne * ringQueue[0].radInPi)) 
						- Math.cos(Math.PI * ringQueue[0].currentRadInPi) * ringQueue[0].radius;
						var ytogo:Float = Math.sin(Math.PI * ringQueue[0].currentRadInPi) * ringQueue[0].radius  
						- ringQueue[0].radius * Math.sin(Math.PI * (ringQueue[0].currentRadInPi + outOfOne * ringQueue[0].radInPi));
						relMove(Std.int(xtogo + xtogostore2), Std.int(ytogo + ytogostore2));
						ringQueue[0].radInPiLeft -= outOfOne * ringQueue[0].radInPi;
						ringQueue[0].currentRadInPi += outOfOne * ringQueue[0].radInPi;
						var temp:Bool;
						if (Std.int(xtogo + xtogostore2) != 0) {
							temp = true;
						} else {
							temp = false;
						}
						xtogostore2 += xtogo - Std.int(xtogo);
						xtogostore2 -= temp ? Std.int(xtogostore2) : 0;
						if (Std.int(ytogo + ytogostore2) != 0) {
							temp = true;
						} else {
							temp = false;
						}
						ytogostore2 += ytogo - Std.int(ytogo);
						ytogostore2 -= temp ? Std.int(ytogostore2) : 0;
						ringQueue[0].elapsed = Conductor.songPosition;
					}
				}
			}
			/*for (i in 0...strumLineNotes.length) {
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (FlxG.save.data.downscroll)
					daNote.y = (strumLineNotes.members[(daNote.mustPress ? 4 : 0) + daNote.noteData].y
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					FlxG.sound.music.stop();
					if (PlayState.SONG.song.toLowerCase() == 'neon-nits') {
						LoadingState.loadAndSwitchState(new VideoState('assets/videos/scenefinal.webm', new PlayState()), true);
					} else {
